...
def pick_and_place_tray(detected_data, slot_id):
    ...
    node_.get_logger().info(f"   [Step 4/8] 접근 위치로 이동: {approach_pose[:3]}")
    if not safe_movel(approach_pose, "트레이 접근(상공)"):
        node_.get_logger().error(f"   ❌ 접근 실패")
        return False
        
    node_.get_logger().info(f"   [Step 5/8] 하강: {grip_pose[:3]}")
    ...
    node_.get_logger().info(f"   [Step 8a] 공중 이동: {air_pose[:3]}")
    if not safe_movel(air_pose, "트레이 공중 이동"):
        node_.get_logger().error(f"   ❌ 공중 이동 실패")
        return False
        
    node_.get_logger().info(f"   [Step 8b] 배치 하강: {place_pose[:3]}")
...
def main(args=None):
    ...
    node_.get_logger().info("🎯 Order Orchestrator Node")
    node_.get_logger().info("=========================================")
