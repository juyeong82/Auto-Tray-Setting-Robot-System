...
def load_calibration():
    ...
    # 파일명 변경: T_gripper2camera.npy -> T_cam2base.npy
    npy_path = os.path.join(current_dir, "T_cam2base.npy")
    
    if os.path.exists(npy_path):
        T_CAM_TO_BASE = np.load(npy_path)
        node_.get_logger().info(f"✅ Eye-to-Hand Calibration loaded: {npy_path}")
    else:
        node_.get_logger().error(f"❌ Calibration file not found: {npy_path}")
        sys.exit(1)

# [함수 교체] transform_camera_to_base
def transform_camera_to_base(cam_xyz):
    # Eye-to-Hand는 로봇 현재 위치(get_current_posx) 불필요
    if T_CAM_TO_BASE is None: return None
    ...
    try:
        ...
        node_.get_logger().error(f"Transform failed: {e}")
        return None
...
def safe_move_and_pick_item(base_pos, item_name, ref_x_pos, ref_y_pos, rot_rz):
    ...
    # ⬆️ 변경: 집어올린 높이를 반환 (safe_travel_z 대신 실제 높이 사용)
    return True, lift_pose[2]
...
def handle_place_item(request, response):
    ...
    approach_place[2] = lifted_z  # ⬆️ 변경: 집어올린 높이 그대로 사용
    ...
    # 8. 배치 하강
    item_h = ITEM_HEIGHTS.get(item_name, 30.0)  # ⬆️ 변경: ITEM_PLACE_Z_OFFSET → ITEM_HEIGHTS
    ...
    if gripper_manager:
        try:
            ...
            node_.get_logger().warn(f"⚠️ Gripper release failed (continuing): {e}")
    else:
        if gripper:
            try:
