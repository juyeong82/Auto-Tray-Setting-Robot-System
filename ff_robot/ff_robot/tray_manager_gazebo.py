...
class TrayManager:
    def __init__(self):
        ...
        # ========================================
        # [🎛️ 시뮬레이션 환경 좌표 (World 파일 기준)]
        # ========================================
        
        # 1. 트레이 배치 위치 (작업 테이블 위)
        # 로봇 베이스 기준 [X, Y, Z] (mm)
        self.WORK_TABLE_POS = [600.0, 0.0, 100.0] 
        self.WORK_TABLE_ROT = [0.0, 0.0, 0.0]      # [Rx, Ry, Rz]

        # 2. 물품 픽업 위치 (Source Positions) [필수 추가 항목!]
        # 로봇이 물건을 집으러 갈 좌표입니다. [X, Y, Z, Rx, Ry, Rz]
        # Z값은 그리퍼가 물체를 집기 좋은 높이로 설정 (약 20cm)
        self.PICKUP_ZONES = {
            "burger1": [400.0, 500.0, 200.0, 0, 180, 0], 
            "fries":   [400.0, 300.0, 200.0, 0, 180, 0],
            "coke":    [400.0, 150.0, 200.0, 0, 180, 0]
        }
